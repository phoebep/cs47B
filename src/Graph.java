...
public class Graph
{
     ...
     */
    public void prims(String startName){
    	HashMap visited = new HashMap();
    	HashMap edges = new HashMap();
        PriorityQueue<Path> unvisited = new PriorityQueue<Path>();
        ...
        for(Edge e: start.adj){
        	e.dest.prev = start;
        	e.dest.dist = e.cost;
        	unvisited.add(new Path(e.dest, e.cost));
        }
        start.dist = 0;
        
        int nodesSeen = 0;
        while( !unvisited.isEmpty( ) && nodesSeen < vertexMap.size( ) )
        {
        	Iterator<Path> it = unvisited.iterator();
        	while(it.hasNext()){
        		Path p=it.next();
        		System.out.println(p.dest.name + ", " + p.cost);
        	}
            Path curr = unvisited.remove();
            System.out.println(curr.dest.name + ", " + curr.cost);
            if(!visited.containsKey(curr.dest)){
            visited.put(curr.dest, curr.cost);
            if(!edges.containsKey(curr.dest)){
            edges.put((curr.toString() + ", "+ curr.dest.name), curr.cost);
            }
            if(curr.dest.prev != null){
            System.out.println(curr.dest.prev.name + " to "+ curr.dest.name + " (Cost: " + curr.cost + ")");
            }
            Vertex v = curr.dest;
            ...
            for( Edge e : v.adj )
            {
                ...
                if(!visited.containsKey(w) && !edges.containsKey(v.name + ", " + w.name) && !edges.containsKey(w.name + ", " + v.name)){
                	if(distw<unvisited.element().cost){
                		w.prev = v;
                		w.dist = distw;
                		unvisited.add( new Path(w, distw));  
                	}
                }
            }
            }
